...
I am developing a small app to calculate some health metrics from a list of blood markers. Among the metrics, we have the biological age and some scores that predict your risk to cardiovascular diseases. While coding I faced a classic type safety challenge while building the FastAPI processing service. My system needs to support multiple algorithms (Biological age  and the different SCORE2 algorithms). Each algorithm requires its own list of biomarkers.

For instance the SCORE2 algorithm needs basic biomarkers:
...
My initial approach used a unified Pydantic model with optional fields as followed:
...
smoking: bool
is_male: bool # Optional diabetes fields
...
def compute_diabetes(biomarkers: Markers) -> RiskResult: # 💥 Type error: biomarkers.diabetes could be None!
...
if diabetes_markers is not None: # Guaranteed type-safe computation
